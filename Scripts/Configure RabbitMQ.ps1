...
$exchange = Get-RabbitMQExchange -ComputerName $ipAddressOfRabbitMQ -UserName $userName -Password $password `
    | Where-Object { $_.Name -eq $exchangeName}
if ($exchange -eq $null) {
...
$queue = Get-RabbitMQQueue -ComputerName $ipAddressOfRabbitMQ -UserName $userName -Password $password `
...
$productQueueBinding = Get-RabbitMQQueueBinding -Name $queueName -ComputerName $ipAddressOfRabbitMQ -UserName $userName -Password $password `
